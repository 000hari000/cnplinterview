...
abstract class REST_Controller extends \CI_Controller {
    ...
    protected function _prepare_basic_auth()
    {
        ...
        if($uri != 'clogin' && $uri != 'cwebsite'){
            if(isset($http_auth) && $http_auth != '' && $http_auth != false){
                $userToken = $http_auth;
                if($userToken != ''){
                    $userToken = $this->service->en_dy_crypt($userToken,'decrypt');
                    if(isset($userToken) && $userToken != ''){
                        $this->load->model('Login_model','login');
                        $searchArr=array();$searchArr['userId']=$userToken;
                        $userData = $this->login->checkUser($searchArr);
                        if(count($userData)  == 1){   $userData = $userData[0];
                            $this->user_id = $userData['userId'];
                        }else{
                            $res = array();
                            $res['response'] = array();$res['response']['code'] = 401;
                            $res['response']['status'] = 'Failed'; $res['response']['message'] = 'Authendication Failed';
